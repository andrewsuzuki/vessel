...
class MediaHelper {
	...
	{
		if (!$this->file->exists($tmp)) throw new \Exception('File was not uploaded.');
		// TODO: check file size is less than max
		if (!$filename || !is_string($filename)) throw new \InvalidArgumentException('Filename was not valid.');
		...
		if (!$basename) throw new \InvalidArgumentException('Filename cannot start with a period.'); // make sure this filename doesn't start with .
