...
class MediaHelper {
	...
	{
		if (!$this->file->exists($tmp)) throw new \Exception(vest('messages.media.upload.not-uploaded-error'));
		// TODO: check file size is less than max
		if (!$filename || !is_string($filename)) throw new \InvalidArgumentException(vest('messages.media.upload.name-invalid-error'));
		...
		if (!$basename) throw new \InvalidArgumentException(vest('messages.media.upload.name-invalid-error')); // make sure this filename doesn't start with .
